...
# ── CUAS MODIFICATION: CBAM implementation ───────────────────────────────────
# Channel and Spatial Attention Module
# Paper: CBAM: Convolutional Block Attention Module (Woo et al. 2018)
# Purpose: Suppresses cluttered background regions, amplifies UAV target features
# Channel attention asks  "which feature channels matter?"
# Spatial attention asks  "where in the image matters?"
# Both are applied sequentially — channel first, then spatial

class ChannelAttention(nn.Module):
    """Channel attention — reweights feature channels via global pooling + MLP."""
    ...
    def __init__(self, c: int, reduction: int = 16):
        ...
        Args:
            ...
            reduction (int): Reduction ratio for MLP hidden layer. Default 16.
        """
        super().__init__()
        c_reduced = max(c // reduction, 1)  # floor at 1 to avoid zero hidden dim
        self.avg_pool = nn.AdaptiveAvgPool2d(1)   # global average pool → (B, C, 1, 1)
        self.max_pool = nn.AdaptiveMaxPool2d(1)   # global max pool     → (B, C, 1, 1)
        # shared MLP applied to both pooled outputs
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # sum both paths, sigmoid to get per-channel weights in [0, 1]
        scale = self.sigmoid(avg_out + max_out).unsqueeze(-1).unsqueeze(-1)  # (B, C, 1, 1)
        return x * scale                       # reweight input channels


class SpatialAttention(nn.Module):
    """Spatial attention — reweights spatial locations via channel pooling + Conv."""

    def __init__(self, k: int = 7):
        """Initialize spatial attention.

        Args:
            k (int): Kernel size for spatial conv. 7 for large receptive field,
                     use 3 for smaller feature maps. Default 7.
        """
        super().__init__()
        assert k in (3, 7), "kernel size must be 3 or 7"
        # Conv takes 2-channel input (avg + max across channels) → 1-channel attention map
        self.conv = nn.Conv2d(2, 1, k, padding=k // 2, bias=False)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply spatial attention to input tensor."""
        # pool across channel dim to get spatial signatures
        ...
        return x * scale                                # reweight spatial locations
...
class CBAM(nn.Module):
    ...
    Designed for UAV detection in cluttered backgrounds — channel attention
    selects drone-relevant features, spatial attention suppresses background.
    """
    ...
    def __init__(self, c: int, reduction: int = 16, k: int = 7):
        ...
        Args:
            c (int): Number of input channels.
            reduction (int): Channel attention MLP reduction ratio. Default 16.
            k (int): Spatial attention conv kernel size. Default 7.
        """
        super().__init__()
        self.channel = ChannelAttention(c, reduction)
        self.spatial = SpatialAttention(k)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply channel then spatial attention."""
        x = self.channel(x)   # step 1 — which channels matter
        x = self.spatial(x)   # step 2 — where in the image matters
        return x
# ── END CUAS MODIFICATION ─────────────────────────────────────────────────────
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.add = shortcut and c1 == c2
        # ── CUAS MODIFICATION: CBAM attention after first conv ───────────────────
        # Inserted between cv1 and cv2 — attends to relevant spatial regions
        # and channels before the second 3×3 refinement conv
        # CBAM is already defined in block.py — no new import needed
        self.cbam = CBAM(c_)  # operates on hidden channels c_
